...
public class DebateService {
        ...
        try {
            ...
            for (int round = 1; round <= debate.getNumRounds(); round++) {
                ...
                String opponentLastStrategy = conversationHistory.isEmpty() ? null
                        : (String) conversationHistory.get(conversationHistory.size() - 1).getOrDefault("strategy",
                                null);
                ...
                String strategyA = (String) argResponseA.getOrDefault("chosen_strategy", "LOGICAL_ARGUMENT");
                ...
                conversationHistory.add(Map.of("role", "agentA", "content", argumentA, "strategy", strategyA));
                ...
                String strategyB = (String) argResponseB.getOrDefault("chosen_strategy", "LOGICAL_ARGUMENT");

                broadcastEvent(debate.getId(), "turn_complete", Map.of(
                        "bot", "B", "content", argumentB, "strategy", strategyB,
                        "round", round, "persona", agentB.getPersona().name()));

                conversationHistory.add(Map.of("role", "agentB", "content", argumentB, "strategy", strategyB));
                ...
                // Save scores
                Map<String, Object> scoresA = (Map<String, Object>) judgeResponse.getOrDefault("agent_a_scores",
                        Map.of());
                Map<String, Object> scoresB = (Map<String, Object>) judgeResponse.getOrDefault("agent_b_scores",
                        Map.of());
                ...
                Thread.sleep(500); // Pace the debate for live viewing
            }
    ...
    private DebateResponse toDebateResponse(Debate d) {
        return DebateResponse.builder()
                ...
                .winner(d.getWinnerAgent() != null ? d.getWinnerAgent().getName() : (d.getIsDraw() ? "TIE" : null))
                .isDraw(d.getIsDraw())
